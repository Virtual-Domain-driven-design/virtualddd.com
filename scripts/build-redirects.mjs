/**
 * Generate `public/.htaccess` — the redirect and gone map for cutover.
 *
 * Every rule traces back to the Phase 1 URL inventory: 967 URLs were indexed on
 * the WordPress site and 674 have no equivalent in the Astro build. Each of
 * those got a deliberate decision (see REVIEW-2026-07-25.md, step 3) rather
 * than being left to 404.
 *
 * Inputs are all committed, so this is reproducible after WordPress is gone:
...
// The eight WordPress team pages map onto the organisers collection.
const TEAM = {
...
// Nav landing pages that were separate Divi pages on WordPress. The rebuild
...
// The ddd-crew reposts lived under /github-repositories/ with WordPress's own
...
L.push('# Redirect map for the WordPress → Astro cutover. See REVIEW-2026-07-25.md.');
L.push('');
...
section('3. Feeds');
L.push('# WordPress served /feed/ and a feed per post type. They were stale (last');
...
L.push('# WordPress published a page per tag. The indexes read ?tag= and pre-select');
...
L.push('# Content that moved after cutover planning: the old URL keeps working.');
...
L.push('# Uncomment each line the day its page ships (see REVIEW-2026-07-25.md).');
