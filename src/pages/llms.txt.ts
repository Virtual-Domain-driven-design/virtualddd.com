 ...
 * artefact to maintain. See docs/seo.md.
 */
import { getCollection } from 'astro:content';
import type { APIContext } from 'astro';
import { shortDate } from '../lib/dates';
import { SITE_NAME, SITE_TAGLINE } from '../lib/seo';
import { creditsFor, guestsById, isUpcoming, resolveRefs } from '../lib/collections';
...
export async function GET(context: APIContext) {
  ...
  const out = [
    ...
    '> the ddd-crew section is republished under each repository\'s own licence,',
    '> mostly CC BY-SA 4.0, named on every page.',
    '>',
    '> Every page below is also available as markdown: append `index.md` to the URL,',
    `> e.g. ${base}/heuristics/<slug>/index.md. The whole corpus in one file is at`,
    `> ${base}/llms-full.txt.`,
    '',
    '## Start here',
    '',
    ...
    link('/ddd-crew/', 'ddd-crew tools', `${crew.length} community canvases and guides, republished with attribution`),
  ...
  for (const s of stories) {
    const note = [s.data.episode != null ? `episode ${s.data.episode}` : null, creditsFor(s.data, guests).join(', ')]
      .filter(Boolean).join(' — ');
